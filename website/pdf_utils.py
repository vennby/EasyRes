...
def generate_resume_pdf(resume):
    ...
    if personal_info:
        ...
        # Now overlay hyperlinks for LinkedIn and GitHub
        x_cursor = start_x
        for part in contact_parts:
            part_width = p.stringWidth(part, font_main, 12)
            if part == 'LinkedIn' and personal_info.linkedin:
                ...
                p.setFillColor(colors.black)
            elif part == 'GitHub' and personal_info.github:
                p.setFillColor(colors.HexColor('#0074D9'))  # Brighter blue
                p.drawString(x_cursor, y_contact, part)
    ...
    if resume.skills:
        ...
        y = draw_wrapped_text(skills, left_margin+10, y, right_margin-left_margin-20, font_main, 12)
    p.save()
    buffer.seek(0)
    return buffer
